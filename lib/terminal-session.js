...
export default class TerminalSession {
  ...
  get shellPath() {
    ...
    return this._shellPath = this.config.shellPath
      || atom.config.get('terminal-tab-fork.shellSettings.shellPath')
      || process.env.SHELL
      || process.env.COMSPEC;
  }
  ...
  get shellArguments() {
    ...
    return this._shellArguments = this.config.shellArgs
      || atom.config.get('terminal-tab-fork.shellSettings.shellArgs')
      || '';
  }
  ...
  get sanitizedEnvironmentKeys() {
    ...
    return this._sanitizedEnvironmentKeys = this.config.sanitizeEnvironment
      || atom.config.get('terminal-tab-fork.shellSettings.sanitizeEnvironment');
  }
  ...
  getDefaultLocation() {
    return atom.config.get('terminal-tab-fork.defaultLocation');
  }
  ...
  getTitle() {
    if (!atom.config.get('terminal-tab-fork.addPath')) return 'Terminal-1';
    return 'Terminal-2';
  }
